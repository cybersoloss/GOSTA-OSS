...
> **Framework state — version-stamped 2026-05-03.** This template was authored against an earlier framework state (Cowork Protocol ~v3.15) and demonstrates the AFC, six-signal framework, and 8-domain deliberation pattern accurately. The framework has since added deliverable-time disciplines that this template does NOT show explicitly in its example outputs: Verdict Strength Annotation `[cluster-confirmation: N, tier-floor: TX]` per verdict (Plan #8), §12.15 Coverage Limitations Disclosure when coverage is sub-threshold (Plan #7), Evidence Channel Disclosure for Party-X-reception claims (Plan #11'), `[VERDICT-SPLIT-CARRIED]` annotation when intra-cluster splits are carried unresolved (Plan #13), formula-based per-deliverable caps (Plans #17/#20), Issue Ledger row class for rejection-taxonomy disagreements with canonical/secondary structure (Plan #21), and schema literal-embedding in dispatch prompts (Plan #22). The §12.12 Frame Integrity Validation section IS demonstrated. See `cowork/sync-manifest.md` C201–C217 for the change record. The conceptual structure (AFC, six-signal framework, 8-domain deliberation) shown here remains canonical. For a fully current example, see [`my-first-session/`](../my-first-session/) (refreshed 2026-05-03 to current framework state).
