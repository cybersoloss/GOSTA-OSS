...
**U1 Independent Reviewer** — Subagent dispatched at phase-gate decision support and at closeout to audit orchestrator's claims against actual files. No domain model — looks for file-grounding violations, AFC consistency, sycophancy patterns, binding-precondition annotations. Produces PASS / FAIL / PASS-WITH-NOTES verdict. Prompt template at `cowork/templates/independent-reviewer-prompt.md`. Related: Sycophancy, Phase Gate.
