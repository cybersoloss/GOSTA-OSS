...
**Cycle Runner** — The non-interactive Tier 0 execution path for ongoing-scope sessions. Runs one Action Cycle per scheduler-triggered invocation against an already-ratified OD, without a Governor in the conversation. Treats the OD as immutable, escalates Blocking tensions and V-gate BLOCKs to a Governor-bound escalation surface, exits cleanly. Peer to the conversational Execution Session in `cowork/gosta-cowork-protocol.md` §5.2. Defined in `cowork/cycle-runner-protocol.md`. Related: Operating Document, Phase Gate, V-gates.

**Diff Agent** — An environmental-watch agent that compares a source's current state against its last-seen state and emits a change-set the consuming session ingests as environmental signals (per spec §7.14.2). Operationalizes the scanning agent §7.14 names but does not specify at any tier. v0.1 specifies the input contract (source state + watch list + Monitor identity), output contract (change-set with §7.14.2 severity vocabulary), and three primary failure classes (source unreachable → informational; partial diff → significant; corrupt last-seen state → Blocking escalation). Source-type adapter abstraction, full failure-mode taxonomy, and evidence-collection-protocol relationship are explicitly deferred. Defined in `cowork/diff-agent-protocol.md`. Related: Cycle Runner, Environmental Signal, Operating Document.
