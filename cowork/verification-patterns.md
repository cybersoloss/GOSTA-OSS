...
**A second observed-drop class:** plans that propose framework-level promotion of mechanisms based on **single-session demonstration** rather than demand-population evidence. A session that successfully uses a custom mechanism is evidence the mechanism works for that session, not evidence that other sessions need or will use it. Promotion warrants pattern-13-class scope-extension justification anchored in demand-population, not in demonstration-of-success. When the dropped variant's intervention can be served by documentation-of-canonical-location instead of infrastructure-promotion (per Pattern 12 convenience-gap discipline), prefer the lighter intervention. If empirical demand surfaces later (multiple sessions or multiple users reporting need), reconsider with Pattern 7 reconsideration discipline.

**Cumulative observation across drops in this class.** Multiple consecutive plans matching the single-session-demonstration shape have dropped after verification — suggesting the failure mode is high-frequency at the framework-evolution → session-execution → framework-feedback handoff. Two specific anti-patterns repeat: (i) **default-change disguised as mechanism-availability** — where the existing mechanism already supports the desired behavior via opt-in, but the plan proposes flipping the default for one session class without verifying the change harms other session classes; (ii) **promotion-of-pattern disguised as gap-fill** — where a session-level pattern (e.g., a custom policy file, a specific dispatch convention) is treated as a framework gap because it isn't centralized, when in fact the session-level mechanism is the right intervention level (per Pattern 14 session-vs-framework distinction). Both anti-patterns benefit from the existing-mechanism check (Pattern 11) and convenience-gap classification (Pattern 12) catching them before plan-drafting commits cognitive resources (Pattern 15).
