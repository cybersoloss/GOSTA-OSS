...
6. **Post-build shape verification (V2).** After any pool build, embedding generation, or chunking operation: run `python3 cowork/tools/pool-agent.py verify-store --store <path>` (LFS-pointer pre-check + shape + metadata-consistency report — emits an actionable `git lfs pull` diagnostic if the store's binary has not been pulled from LFS, instead of the cryptic numpy pickle-key error chain that ad-hoc `np.load` produces in that state). For non-pool-store artifacts where the verify-store subcommand does not apply, fall back to direct `numpy.load(...).shape` inspection. If `N_embeddings == N_input_files` for non-trivial input sizes, treat as suspicious and present to Governor before proceeding.
