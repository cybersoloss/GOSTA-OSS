...
6. **Populate the OD template.** Fill in `cowork/templates/operating-document.md` using the synthesized structure. Apply the field complexity determination from cowork protocol §4: include [CORE] always, include [ROBUST] if deliberation or A/B testing or ≥Independence Level 2, include [ADVANCED] if metric lag or metric prerequisites.
   ...
   **6a. Per-Deliverable Caps default selection (Plan #20).** When the synthesized scope includes deliberation AND evidence collection (any session that uses position papers, per-criterion evaluation papers, per-regulation analyses, or analogous content-density-variable artifacts), the Per-Deliverable Caps section MUST use formula caps as the default for those artifact classes. Apply the recommended-default rule from `cowork/templates/operating-document.md` § Formula-Based Caps:
   - For `position-*.md` artifacts (deliberation §4.1): default formula `base=4kb + 1.0kb × evidence_items_assigned_to_agent`
   - For per-criterion evaluation papers (vendor / hiring): default formula `base=3kb + 0.8kb × candidate_responses_per_criterion`
   - For per-regulation analyses: default formula `base=5kb + 1.2kb × precedent_citations`

   Fixed-value caps for these artifact classes produce systematic overage friction because content density scales with input scope (cite-then-apply discipline expands content proportional to evidence count). The OD Architect adopts the formula default unless the synthesized scope makes evidence-density variation across agents unlikely (e.g., all agents will be assigned identical-sized evidence packs by design); in that case, the Cap row's Rationale cell records the one-line override justification. The drafted OD should not ship with a fixed cap on these artifact classes (e.g., `position-*.md | 4 KB`) absent an explicit override rationale.
