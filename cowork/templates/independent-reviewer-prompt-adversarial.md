...
6. **Classification delta articulation** — for each `ENHANCEMENT-OF-X` and `OVERLAPS-X-AND-Y` classification (or session-specific equivalents), verify the entry articulates the specific delta: what the candidate adds to / changes about / replaces in the named reference feature. Boilerplate ("extends X capability") or absent delta ("ENHANCEMENT-OF Contract Vault" with no description of the enhancement) fails this check. Candidates classified as ENHANCEMENT-OF where the entry is verbatim or near-verbatim restatement of the reference feature are mis-classifications and should be flagged as `[CLASSIFICATION-DELTA-MISSING: classification=<label>, reference=<feature-name>]`. Per Plan #35-light: this check is not enforced via separate framework discipline rule; it is an explicit U1-adversarial enumeration item because the failure mode is per-instance (doesn't cascade) and U1 catch is the right intervention level (not earlier-catch deliberation-time check, per verification-patterns Pattern 5 cascading-vs-per-instance sub-pattern).

7. **Count consistency.** For each summary count claimed in a deliverable section header, table caption, or aggregate statistic (e.g., "ADOPT-MEDIUM (12)", "Total: 24 entries", "8 cross-cluster + 50 adversarial = 58"), verify the count matches the actual number of entries enumerated in the corresponding primary section of the same file. If the deliverable references counts from adjacent deliverables (e.g., a decisions log citing "DEC-PG-3 dispositioned 16 ADOPT cross-cluster consensus" while the synthesis report claims 19 in Section A), flag the cross-deliverable mismatch when the deliverable EXPLICITLY cites another deliverable's count. Per-instance flag: `[COUNT-CONSISTENCY-MISMATCH: claimed=X, actual=Y, source=<file:section>, primary-evidence=<file:section>]`. Resolution paths (Coordinator + Governor decide): (a) update summary count to match primary section; (b) add missing entries to primary section to match summary count; (c) annotate the count as "summary-count includes cross-listed entries" if a reconciliation note explains the discrepancy. Scope: same-file consistency mandatory; cross-deliverable consistency only when one deliverable explicitly cites another deliverable's count. Per Plan #37: this check addresses an empirical blind spot — v5's adversarial U1 produced 45 findings but did not catch a 3-way ADOPT-MEDIUM count disagreement (8 in DEC-PG-3 / 12 in synthesis-report Recommendations §1 / 13 in adoption-list primary section). Without explicit enumeration, U1 dispatches systematically miss summary-vs-primary count drift.
