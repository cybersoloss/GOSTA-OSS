...
**Recommended-default rule (Plan #20).** For deliberation-enabled sessions where evidence collection is in scope, **formula caps are the recommended default for `position-*.md` artifacts** (and analogous content-density-variable artifact classes). Sessions should adopt the recommended formula above unless evidence-density variation across agents is not expected (e.g., all agents assigned equal-size evidence packs and content density is intrinsically bounded). Fixed-value caps for content-density-variable artifacts produce systematic overage friction; the formula calibration eliminates this. OD authors should explicitly justify deviation from formula default with a one-line rationale in the Cap row's Rationale cell.
...
| Build artifact shape for pool-N (V2) | TAC-N evidence collection | `python3 cowork/tools/pool-agent.py verify-store --store <path>` (LFS-pointer + shape + consistency); flag suspicious `N_emb == N_files` for non-trivial inputs | WARN; BLOCK if downstream depends on chunk-level discrimination | Rebuild with `index-doc --heading-level N` |
