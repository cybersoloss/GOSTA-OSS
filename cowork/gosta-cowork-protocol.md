...
2a. **U1 independent-reviewer dispatch at phase-gate decision support (mechanizable-discipline subset, conditional on V8 PASS).** Before presenting the structured Phase Gate Request to the Governor for decision, the AI dispatches an independent-reviewer subagent per `cowork/templates/independent-reviewer-prompt.md` with `boundary type: phase-gate-decision-support`. The reviewer reads the AI's recommendation + the source files cited in the recommendation; verifies file-grounding integrity (does the AI's claim actually appear in the cited file? if quoted, verbatim?); applies §14.3.9 sycophancy patterns to the AI's recommendation. The reviewer's structured findings report is included in the Phase Gate Request as a separate field — the Governor reviews findings before deciding advance / iterate / restructure / advance-with-degradation. Hard dependency: V8 PASS required (reviewer is a subagent dispatch). If V8 = N/A or BLOCK, the AI records `U1 phase-gate review: skipped — V8 dependency not met` in the Phase Gate Request and the Governor proceeds without independent review (residual training-distribution drift risk — the intrinsic LLM property where assistant outputs regress toward training defaults under attention load — explicitly acknowledged at the gate).
...
**U1 independent-reviewer dispatch at closeout (mechanizable-discipline subset, U1 of Plan #3):**

After V6 Layer B passes (mechanical check), dispatch an independent-reviewer subagent per `cowork/templates/independent-reviewer-prompt.md` with `boundary type: closeout-claim`. The reviewer reads the primary orchestrator's claims about populated artifacts + the actual artifacts; verifies citation grounding (does the orchestrator's claim that "learnings.md is populated with N validated patterns" match the file's actual content?); applies §14.3.9 sycophancy patterns to the closeout summary itself.

- **Hard dependency:** U1 requires V8 PASS (reviewer is a subagent dispatch). If V8 = N/A or BLOCK, skip U1 and record `U1 closeout review: skipped — V8 dependency not met` in 00-BOOTSTRAP.md. Acknowledge the residual risk.
- **Reviewer findings:** Governor reviews the structured findings report (per template output format) and decides accept / accept-with-amendments / reject-and-iterate. Reviewer does not directly block — Governor disposes.
- **Cost:** one Agent dispatch + non-trivial elapsed time on the closeout turn. This is the price of mechanizing the unmechanizable subset (file-grounding integrity, AFC content quality, sycophancy detection beyond what V6 Layer B catches mechanically).

**Enforcement:** The completion signal cannot be emitted until steps 1-8 are complete AND V6 Layer B passes on all closeout-mandated artifacts AND U1 reviewer findings are reviewed by Governor (or U1 explicitly skipped with V8-dependency acknowledgment). Template stubs surviving to session close — whether detected by the file-audit table or by V6 Layer B mechanical check or by U1 reviewer — is a protocol violation.
...
**U1 independent-reviewer dispatch at deliverable production (Plan #3 mechanizable subset, conditional on V8 PASS).** After the deliverable agent's self-attestation (§12.13) and §12.12 self-checks pass, but BEFORE final commit (Stage B publication for finite analytical scopes), dispatch an independent-reviewer subagent per `cowork/templates/independent-reviewer-prompt.md` with `boundary type: deliverable-afc`. The reviewer reads the draft deliverable + the AFC declaration from the OD; performs Check class A (file-grounding citation audit) on the deliverable's claims; performs Check class B (AFC frame-integrity content quality — verifies frame-audit sections cite specific deliverable text rather than producing boilerplate); performs Check class C (§14.3.9 sycophancy patterns in the deliverable). The reviewer's structured findings report is presented to the Governor alongside the deliverable for review. The Governor decides accept / accept-with-amendments / reject-and-iterate based on the reviewer's findings + own review. Hard dependency: V8 PASS required. If V8 = N/A or BLOCK, the deliverable agent records `U1 deliverable review: skipped — V8 dependency not met` in the deliverable's revision trail and surfaces the residual risk to the Governor explicitly.
