...
│
├── session-logs/                ← Episodic memory. Per-orchestrator-conversation narrative; optional per §5.5 step 6 (audit-trail union covers phases via multiple canonical surfaces).
│   └── session-[NNN].md
│
...
**Step 6 — Write session log (optional, recommended).** Optionally create `session-logs/session-NNN.md` capturing: date, session type, what was discussed/decided/produced, what domain models were consulted, any framework feedback items discovered. Per §5.5 step 6, session-logs are the per-orchestrator-conversation narrative format and are optional when other canonical audit-trail surfaces (decisions log, signals, deliberation/, deliverables/, learnings.md, framework-feedback, debug-logs/orchestrator-trace.md) cover the phase. Sessions running across multiple orchestrator-conversation chunks may write one session-log per chunk, one per phase, or none — the closeout audit verifies the canonical-surface union, not session-logs/ specifically.
...
1. At phase completion, the AI MUST produce a structured Phase Gate Request:
   ...
   **Pre-Flight Validation Gate Results (from GOSTA §8.7):** Per-invariant outcomes at this phase boundary.
   ...
   - V5 Runtime Import Verification: [PASS — import-test succeeded in orchestrator runtime | BLOCK — module(s) missing: list | N/A — only checked at first tool invocation per session]
   - V6 Declared Artifact Existence and Population: [PASS — Layer A (every CLAUDE.md / OD / scope-declared artifact present with non-zero content) AND Layer B (templated artifacts populated; no remaining `[POPULATE: ...]` sentinels; per-section word counts above floor) | BLOCK — missing-or-empty OR template-shaped list with disposition: create-or-populate-now / defer-with-reason / remove-from-declaration. **At closeout phase gate, V6 fires explicitly on closeout-mandated artifacts** (learnings.md, gosta-framework-feedback.md, plus any session-specific closeout deliverables). `session-logs/session-NNN.md` is NOT in the V6-mandated set per §5.5 step 6 (audit-trail union covers phases via multiple canonical surfaces; session-logs/ is one optional surface). Closeout cannot complete with template-shaped closeout artifacts.]
   ...
   - V8 Subagent Dispatch Capability Smoke-Test: [PASS — probe subagent's marker file observable from orchestrator AND no-op tool calls clean in subagent runtime | BLOCK — environment-mismatch surfaced (path-resolution / permission scope / tool callability differs from orchestrator); disposition: reroute-path / extend-sandbox / collapse-to-orchestrator | N/A — only checked at bootstrap when subagent dispatch declared in scope/OD/CLAUDE.md, OR session does not declare subagent dispatch]
   - V9 Inheritance Framework-Residue Audit: [PASS — framework-residue list empty (no tokens in inherited artifact absent from inheriting session's declarations) | WARN — N residue tokens surfaced; per-token Governor disposition: update / acknowledge-as-historical / extend-declarations | N/A — only checked at Phase 1 entry when session inherits artifacts from prior session or example library, OR session does not inherit]
...
2a. **U1 dual-role independent-reviewer dispatch at phase-gate decision support (mechanizable-discipline subset, conditional on V8 PASS; dual-role per Plan #30).** Before presenting the structured Phase Gate Request to the Governor for decision, the AI dispatches the U1 independent-reviewer in two distinct roles for analytical scopes (recommended-default for finite analytical scopes; constructive-only acceptable for low-stakes operational scopes per the constructive template's standalone-mode behavior). **U1-adversarial** (per `cowork/templates/independent-reviewer-prompt-adversarial.md`, `boundary type: phase-gate-decision-support`) enumerates weaknesses in the AI's recommendation: ungrounded citations, tier-inflation, claims that don't survive evidence read-through, sycophancy patterns. AI dispositions each finding (`addressed-via-revision` / `rebutted-in-writing` / `acknowledged-as-limitation`). **U1-constructive** (per `cowork/templates/independent-reviewer-prompt-constructive.md`, same boundary type) verifies pass conditions on the revised recommendation: each adversarial finding's disposition holds, file-grounding holds, signal integrity holds. The constructive verification report is included in the Phase Gate Request as a separate field — the Governor reviews findings + verification before deciding advance / iterate / restructure / advance-with-degradation. Hard dependency: V8 PASS required for both dispatches. If V8 = N/A or BLOCK, the AI records `U1 phase-gate review: skipped — V8 dependency not met` in the Phase Gate Request and the Governor proceeds without independent review (residual training-distribution drift risk — the intrinsic LLM property where assistant outputs regress toward training defaults under attention load — explicitly acknowledged at the gate).
...
**Audit-trail union coverage:**
6. The session's audit trail is the union of multiple canonical surfaces, not a single file. Verify that the union of canonical surfaces covers all conducted phases of the scope. Canonical surfaces include: `decisions/governor-decisions.md` (Governor dispositions, phase-gate decisions), `signals/` (orchestration-event records per phase), `deliberation/[DELIB-NNN]/` (deliberation artifacts when phases include deliberation), `deliverables/` (published outputs from synthesis or production phases), `health-reports/` (phase-gate request artifacts), `learnings.md` (closing scope-conclusion narrative), `gosta-framework-feedback.md` (framework-feedback entries), `debug-logs/orchestrator-trace.md` (orchestration checkpoint trace), and `session-logs/session-NNN.md` (per-orchestrator-conversation narrative). No single surface is mandatory when others cover the phase. The `session-logs/` directory carries the per-orchestrator-conversation narrative format — sessions may write entries there at any cadence the orchestrator finds useful (one per conversation chunk, one per phase, or none when other surfaces are sufficient). Template stubs in any populated artifact count as missing for that artifact; complete absence of a phase across all canonical surfaces is not acceptable. Populate gaps from memory before closeout where any phase has no canonical-surface representation — partial reconstruction is acceptable, complete absence is not.
...
**V6 populated-check at closeout (from GOSTA §8.7.3 V6 Layer B):** As part of step 8, run the V6 Layer B mechanical check on every templated closeout artifact in the table above:
```bash
# (1) Sentinel removal — sentinels must be replaced with session content
for artifact in <closeout-mandated artifacts>; do
  count=$(grep -c "\[POPULATE:" "$artifact" 2>/dev/null || echo 0)
  [ "$count" = "0" ] || echo "V6_LAYER_B_FAIL: $artifact has $count remaining sentinels"
done
# (2) Minimum-content check — formerly-sentineled sections meet word-count floor (default 20)
```
A V6 Layer B failure at closeout maps to STUB / template-only status in the audit table and is blocking. This catches the operational-truth gap where a closeout artifact is non-zero (template scaffolding has content; Layer A passes) but session-specific population was never executed (sentinels remain; Layer B catches). The closeout-fidelity gap is the most common surface for this failure mode and §8.7's V6 explicitly fires here.

**Coverage Limitation Disclosure check (§12.15):** As part of step 8, also verify the §12.15 deliverable-disclosure rule. Read the session's evidence manifest (typically `osint/evidence-manifest.md`); inspect coverage-mode disposition records. If any sub-threshold coverage outcome is recorded (any WARNING or SCARCITY-ACKNOWLEDGED entry), grep each published deliverable in `deliverables/` for a "Known Coverage Limitations" section header. Failure mode: any deliverable depending on the affected domains lacks the section. A §12.15 failure at closeout is blocking with the same disposition options as V6 Layer B (create-or-populate-now / defer-with-reason / remove-from-declaration). When the manifest carries no sub-threshold coverage outcomes, the §12.15 check passes trivially — the section is not required.

**U1 dual-role independent-reviewer dispatch at closeout (mechanizable-discipline subset, U1 of Plan #3; dual-role per Plan #30):**

After V6 Layer B passes (mechanical check), dispatch the U1 independent-reviewer in **two distinct roles** as separate subagent dispatches. The dual-role design (adversarial enumerates; constructive verifies on revisions) prevents the rubber-stamping bias that emerges when one reviewer is asked to both find weaknesses and verify pass — those framings produce conflicting incentives in the same generation. Splitting them gives each reviewer a coherent role.

**Sequencing:**

1. **Adversarial U1** — Dispatch per `cowork/templates/independent-reviewer-prompt-adversarial.md` with `boundary type: closeout-claim`. The adversarial reviewer's sole job is to **enumerate weaknesses** in the orchestrator's closeout claims + actual artifacts. Output: structured findings report with severity (high/medium/low) and category (ungrounded-citation, tier-inflation, verdict-doesnt-survive, classification-issue, domain-attribution-issue, sycophancy-pattern, rejection-reason-vague, coverage-gap-omitted, etc.).

2. **Orchestrator disposition of adversarial findings** — Each finding dispositioned as `addressed-via-revision` (revise the artifact and document the revision), `rebutted-in-writing` (record rebuttal in `deliverables/u1-adversarial-rebuttals.md` with finding-ID and resolution), or `acknowledged-as-limitation` (document in §12.15 Coverage Limitations Disclosure or equivalent visibility surface). Unaddressed findings BLOCK closeout completion.

3. **Constructive U1** — Dispatch per `cowork/templates/independent-reviewer-prompt-constructive.md` with `boundary type: closeout-claim`, the revised artifacts, AND the adversarial findings + dispositions. The constructive reviewer's sole job is to **verify pass conditions** on the revised output: each adversarial finding's disposition is verified (revision actually addresses gap, rebuttal actually addresses claim, limitation actually visible), AFC frame-integrity content quality holds, signal integrity holds, V6 closeout-mandated artifact population holds. Output: structured verification report with aggregate verdict PASS / PASS-WITH-NOTES / FAIL.

- **Hard dependency:** Both dispatches require V8 PASS (reviewer is a subagent dispatch). If V8 = N/A or BLOCK, skip U1 and record `U1 closeout review: skipped — V8 dependency not met` in 00-BOOTSTRAP.md. Acknowledge the residual risk.
- **Reviewer findings:** Governor reviews the structured outputs (adversarial findings + dispositions + constructive verification) and decides accept / accept-with-amendments / reject-and-iterate. Neither reviewer directly blocks — Governor disposes. In high-autonomy mode (per session governor-policy.md), constructive U1's PASS verdict pre-authorizes publish-approval; FAIL verdict triggers revision rounds (typically up to 2 autonomous rounds before ESCALATE).
- **Cost:** two Agent dispatches + non-trivial elapsed time on the closeout turn. Dual-role overhead (~2x single-role cost) is the price of separating enumeration-as-incentive from verification-as-incentive in independent-reviewer dispatch.
- **Recommended-default scoping:** dual-role is the default for finite analytical scopes (deliberation-enabled, AFC-declared) where rubber-stamping bias is the dominant risk; sessions with constrained dispatch budgets or low-stakes operational scopes may dispatch constructive U1 standalone (skipping adversarial enumeration), in which case the constructive reviewer's Check class A reduces to PASS-by-default per the constructive template's documented standalone-mode behavior.

**Enforcement:** The completion signal cannot be emitted until steps 1-8 are complete AND V6 Layer B passes on all closeout-mandated artifacts AND U1 dual-role review completes with all adversarial findings dispositioned + constructive U1 verdict reviewed by Governor (or U1 explicitly skipped with V8-dependency acknowledgment). Template stubs surviving to session close — whether detected by the file-audit table or by V6 Layer B mechanical check or by U1 reviewer — is a protocol violation. Unaddressed adversarial findings surviving to publish — whether unrevised, unrebutted, or undocumented as limitation — is a protocol violation.
...
**Verdict Strength Annotation propagation (from Deliberation Protocol §4.4) `[CORE]`:** When a deliverable presents per-candidate verdicts with verdict-band labels (HIGH/MEDIUM/LOW or any session-specific verdict vocabulary), each verdict must carry the structured strength annotation produced at synthesis: `cluster-confirmation: N, tier-floor: T<X>`. The deliverable agent transcribes the annotation from the synthesis report's Verdict Strength Annotation field; does not re-derive. Without this annotation, single-source-uncontested verdicts and cross-cluster-confirmed verdicts within the same band are indistinguishable to readers — band-label confidence is over-claimed. When a verdict shipped under deliberation-protocol §3.3 Governor override path (b) — an intra-cluster verdict-band split exceeding the session's declared convergence threshold remained open at termination — the synthesis annotation includes the conditional `[VERDICT-SPLIT-CARRIED]` field with split width and disagreeing agents/clusters; the deliverable transcribes this field inline alongside the verdict-band label so readers see the unresolved split at-a-glance rather than hunting for it in body-text footnotes. Failure mode: any verdict-band entry in deliverable that lacks the annotation, OR any verdict shipped via §3.3 override path (b) without the `[VERDICT-SPLIT-CARRIED]` field surfaced inline. Severity: MATERIAL — same disposition as `[FRAME-DRIFT]` (revision before Governor acceptance). When the deliberation produced no per-candidate verdicts (single-recommendation synthesis, single-agent assessment), the annotation does not apply and the deliverable agent records this in the deliverable's metadata.

**Classification-against-reference reporting discipline `[CORE]`:** When a deliverable classifies candidates against a reference document (a product spec, a regulatory framework, a prior-session OD, a baseline architecture, a job description, or any other named source-of-truth artifact), two reporting requirements apply at deliverable production:

1. **Classification preamble with operational definition AND negative-space disclaimer.** The deliverable's classification preamble must declare three things: (a) the reference document being classified against, with its named identifier and section scope; (b) the operational meaning of each classification label against that reference (e.g., "NEW = capability has no functional analog in [reference §X]"); and (c) **what each label does NOT mean** (negative-space declaration — e.g., "NEW does NOT imply market-novelty, competitive uniqueness, or unprecedented industry surfacing; the classification is scoped to absence-from-reference, not absence-from-market"). Without the negative-space declaration, surface language in deliverable summary statistics ("wholly NEW," "unprecedented," "novel") tends to drift toward stronger semantic content than the operational definition supports. Failure mode: classification preamble declares (a) and (b) but omits (c); reader infers stronger novelty than the scope licenses.

2. **Domain-attribution breakdown alongside count, with domain-name-key.** Each classification bucket's headline statistic must include per-domain-attribution breakdown showing which domain models contributed candidates to that bucket. Surface format: `[bucket label]: N total ([domain-1]: m, [domain-2]: k, ...)`. Domain-attribution surfaces concentration patterns that aggregate counts hide — e.g., a hypothetical "7 NEW (Domain-A: 3, Domain-B: 1, Domain-C: 1, Domain-D: 1, Domain-E: 1)" reveals that 3 of 7 NEW entries came from one domain, which the headline "7 NEW" alone conceals. **Domain-name-key requirement:** when domain identifiers are abbreviations or codes, the deliverable must include a domain-name-key — either as a glossary block at the top of the deliverable or as first-occurrence inline expansion (e.g., "Domain-A (Financial Modeling): 3"). Without the key, casual readers see opaque IDs and the breakdown's information is lost. Presentation discipline for large breakdowns: when a bucket has contributions from >5 domains, present top-5 by count descending with "(... and N more domains)" tail. Sessions without classification, with single-domain rosters, or producing single-bucket outputs (binary GO/NO-GO) skip this rule.

Both rules are deliverable-time reporting discipline at the consumption point — not classification-mechanism changes. They make existing classifications more legible to readers without altering how candidates are classified. Failure mode for either rule: MATERIAL — same disposition as `[FRAME-DRIFT]` (revision before Governor acceptance).

**Evidence Channel Disclosure `[CORE]`:** When a deliverable carries claims about a specific external party's reception (acceptance, uptake, preference, willingness-to-pay, regulator-approval, candidate-fit, auditor-defensibility, or any analogous Party-X-reception claim), the deliverable must include an Evidence Channel Disclosure section that:

1. **Names the evidence channels used.** A structured list of source classes contributing to the deliverable's verdicts (e.g., analyst-published reports, statistical breach data, regulatory-published guidance, internal model reasoning, online search via WebSearch, customer interviews, pilot data, win/loss data, vendor RFP responses, candidate references). Source classes are domain-agnostic — sessions instantiate the list per their evidence collection.

2. **Identifies external-party-reception claims that rest on analytical-construction.** For each external party referenced in the goal language (prospect / regulator / candidate / vendor / auditor / etc.), the deliverable explicitly states whether claims about that party's reception are (a) **analytical-construction** — derived from second-order evidence (analyst reports, prior precedent, model reasoning) without direct Party-X signal, or (b) **empirically-validated** — grounded in ≥1 evidence channel sourced from or about Party-X (Party-X interviews, pilot/usage data, prior-engagement record, Party-X-published material). Claims that mix the two channels are stated as such with the proportions.

**Rationale.** Goal language frequently makes claims about specific external parties' reception ("prospects will adopt this," "regulators will accept this," "candidates will fit this role"). Evidence collection may consist entirely of second-order analyst-consumed sources without any Party-X-sourced channel. The framework's existing factual-claim-grounding rule (§14.3.11 Check 8) verifies citations exist; it does not verify that citations match the claim-class. A reader of the deliverable seeing high-confidence verdicts about Party-X reception may infer empirical Party-X validation from the band-label confidence. Without this disclosure, that inference goes unchecked. The disclosure is informational — it does not change verdicts; it surfaces what the verdicts rest on so readers can calibrate trust before acting.

**Generic abstraction.** The rule operates on the concept *"claims about Party-X's reception require Party-X-sourced or Party-X-about evidence; absent such evidence, the claim is analytical-construction and must be disclosed as such."* Sessions instantiate Party-X per their domain — buyer/prospect for product evaluation, regulator for regulatory analysis, candidate for hiring, vendor for procurement, auditor for compliance assessment, etc.

**Failure mode.** Deliverable carries Party-X-reception claims (in goal language, in verdict descriptions, in summary statistics) without an Evidence Channel Disclosure section, OR the section is present but writes generic boilerplate ("evidence from various sources") that does not identify analytical-construction vs empirically-validated claim classes. Severity: MATERIAL — same disposition as `[FRAME-DRIFT]` (revision before Governor acceptance).

**When the rule does not apply.** Sessions whose goal language makes no external-party-reception claims (internal architectural decisions, capability inventories without external-acceptance language, technical-feasibility assessments) skip the rule. The Coordinator records the omission with a one-line rationale in the deliverable's metadata.
...
**U1 dual-role independent-reviewer dispatch at deliverable production (Plan #3 mechanizable subset, conditional on V8 PASS; dual-role per Plan #30).** After the deliverable agent's self-attestation (§12.13) and §12.12 self-checks pass, but BEFORE final commit (Stage B publication for finite analytical scopes), dispatch the U1 independent-reviewer in two distinct roles. **U1-adversarial** (per `cowork/templates/independent-reviewer-prompt-adversarial.md`, `boundary type: deliverable-afc`) enumerates weaknesses: ungrounded citations, tier-inflation, verdict-doesnt-survive, classification-issues against reference document, domain-attribution-issues, sycophancy patterns, vague rejection reasons, omitted coverage gaps. Output: structured findings report. The deliverable agent dispositions each finding (revise the deliverable / record rebuttal in `deliverables/u1-adversarial-rebuttals.md` / acknowledge as limitation in §12.15 Coverage Limitations Disclosure). **U1-constructive** (per `cowork/templates/independent-reviewer-prompt-constructive.md`, same boundary type) verifies pass conditions on the revised deliverable: each adversarial finding's disposition holds, AFC frame-integrity content quality holds (Check class B: stance / output verb / failure mode / prohibited frame all cited with specific deliverable text), signal integrity holds. Output: aggregate verdict PASS / PASS-WITH-NOTES / FAIL. Both dispatches' outputs are presented to the Governor alongside the deliverable for review. The Governor decides accept / accept-with-amendments / reject-and-iterate based on the dual-role outputs + own review. Hard dependency: V8 PASS required for both dispatches. If V8 = N/A or BLOCK, the deliverable agent records `U1 deliverable review: skipped — V8 dependency not met` in the deliverable's revision trail and surfaces the residual risk to the Governor explicitly. Recommended-default scoping: dual-role for finite analytical scopes (deliberation-enabled, AFC-declared); constructive-only acceptable for low-stakes operational scopes.
...
2b. **Inherited-artifact framework-residue audit (V9) — conditional.** When the session inherits artifacts (domain models, scope decisions, deliberation rosters, templates, reference artifacts) from a prior session or from a different framework version, the AI extracts framework-concept tokens from the inheriting session's OD + scope + CLAUDE.md (structural identifiers like OBJ-N / STR-N / TAC-N / ACT-N / DEC-PG-N / DEC-N / DELIB-N / G-Name; framework-version markers declared in the session's CLAUDE.md or OD) and from each inherited artifact, then computes the set difference: tokens in inherited artifact NOT in inheriting session's declarations. The result is the framework-residue list. V9 is the inverse direction of V7: V7 catches coverage gaps (missing what we need); V9 catches residue (carrying what we don't need). Failure: WARN at Phase 1 entry. The Governor classifies each residue token (framework-version-mismatch / concept-version-mismatch / neutral content) and disposes per token: (a) update inherited artifact to remove or rename, (b) treat as historical context only with explicit OD §Decision History acknowledgment, or (c) extend inheriting session's declarations to include the residue concept. Phase 1 entry proceeds either way; the Governor's per-token disposition is logged before downstream phases consume the inherited artifact. Sessions without inheritance declaration skip V9.

2a. **Subagent dispatch capability smoke-test (V8) — conditional.** If the session declares subagent dispatch in scope, the OD, or CLAUDE.md, the AI dispatches a probe subagent that (a) writes a marker file at the session directory and (b) runs a no-op call against any declared-for-subagent tool. The orchestrator independently verifies via Read tool that the marker file exists at the reported path. V8 fails if: the probe reports `permission denied` / `Bash blocked` / `Write denied` / sandbox-path-resolution error during file creation; the no-op tool call raises an environment-specific error not seen in V5; or the orchestrator's independent Read does not see the marker file at the reported path. Three classes of failure surface: path-resolution boundaries (workspace symlinks not mounted in subagent sandbox), permission-grant scope (orchestrator grants do not propagate into subagent sandbox), declared-tool callability differences (tool imports cleanly in orchestrator, not in subagent runtime). Failure: BLOCK at first phase that depends on subagent dispatch. Mitigation paths: reroute-path / extend-sandbox / collapse-to-orchestrator. Sessions without subagent-dispatch declaration skip V8.
...
5. **Build artifact shape verification (V2).** After any pool, index, or embedding build, the AI inspects the produced artifact's shape via `python3 cowork/tools/pool-agent.py verify-store --store <path>` (LFS-pointer pre-check + shape + metadata-consistency report; exit 0 clean, 1 on LFS pointer or missing files, 2 on consistency WARN), or falls back to `numpy.load('<embeddings>.npy').shape` for non-pool-store artifacts. If `N_embeddings == N_input_files` for non-trivial inputs (default threshold 20-50KB plain-text), the AI flags as suspicious and verifies the build subcommand was the intended one. Failure: WARN; BLOCK if downstream depends on chunk-level discrimination.
...
7. **Declared artifact existence and population (V6).** For every artifact named in CLAUDE.md, OD §Decision History, or scope as a phase deliverable, the AI runs two layers of mechanical test. *Layer A — existence:* `test -s <path>`. Missing-or-empty artifacts BLOCK phase exit unless explicitly deferred with a Governor-acknowledged reason in OD §Decision History. *Layer B — populated-where-templated:* for each artifact whose template carries `[POPULATE: <description>]` sentinel markers, run `grep -c "\[POPULATE:"` (must return 0) and verify each formerly-sentineled section meets the per-section word-count floor (default 20 words). For artifacts where sentinel markup does not fit, fall back to template-fingerprint diff (hash baseline at session-init; compare against current artifact; fail if <30% of template lines changed). Layer B failures BLOCK phase exit identically to Layer A. **At the closeout phase gate, V6 fires explicitly on closeout-mandated artifacts** (e.g., `learnings.md`, `gosta-framework-feedback.md`, plus session-specific closeout deliverables); closeout cannot complete with template-shaped closeout artifacts. `session-logs/session-NNN.md` is NOT V6-mandated per §5.5 step 6 (the audit-trail union covers phases via multiple canonical surfaces; session-logs/ is one optional surface, not a required artifact). The Governor sees the missing-or-template-shaped list and chooses per artifact: create-or-populate-now / defer-with-reason / remove-from-declaration.
...
### 12.15 Coverage Limitation Disclosure (from GOSTA §14.8 four-outcome model) `[CORE]`

When evidence collection produces sub-threshold coverage outcomes (WARNING or SCARCITY-ACKNOWLEDGED per spec §14.8), the deliverable consumer must be able to see which domains had incomplete evidence and why — otherwise the deliverable presents verdicts whose confidence is implicitly weaker than the surface text suggests.

**Rule.** When the session's evidence quality audit produced any coverage-mode WARNING or SCARCITY-ACKNOWLEDGED outcome (including those that auto-resolved at lower-stakes audit boundaries), the deliverable agent must include a "Known Coverage Limitations" section in each published deliverable that depends on the affected domains. The section names each affected domain, its measured coverage percentage, the Governor's disposition for that WARN (accept-as-evidence-base-limitation / re-collected / verdict-downgraded), and the named structural cause (where one was identified at audit-time).

**Schema (illustrative).** A two- or three-column table with columns: domain identifier, coverage percentage, disposition + structural cause. Sessions may extend the schema with verdict-impact notes (which deliverable verdicts the limitation bounds) where useful.

**Source of truth.** The deliverable's "Known Coverage Limitations" section is populated from the evidence manifest's coverage-disposition records, which in turn carry the four-outcome classification produced at audit-time per spec §14.8. The deliverable agent does not re-classify; the agent transcribes from the manifest. If the manifest is missing dispositions for any sub-threshold domain, that is a §12.13 attestation log gap — surface to Governor before publication.

**When the rule does not apply.** Sessions whose evidence quality audit produced no sub-threshold outcomes (all PASS) have no limitations to disclose; the section is omitted. Sessions without coverage-mode audit (e.g., directional-mode-only assessments) have no coverage outcomes to disclose; the section is omitted unless the directional audit also produced limitations the deliverable consumer should see.

**Closeout enforcement.** §5.5 Closeout File Audit checks whether the session's evidence manifest carries any sub-threshold coverage outcomes; if yes, the closeout audit verifies that each published deliverable carries a "Known Coverage Limitations" section. Missing-section is a closeout BLOCK with the same disposition options as V6 Layer B (create-or-populate-now / defer-with-reason / remove-from-declaration).

**Rationale.** Spec §14.8's four-outcome model exists precisely so the structural-vs-recoverable distinction is preserved. Without §12.15, the manifest carries the distinction but the deliverable does not, and the consumer is left to assume verdicts are equally confident across all domains. §12.15 closes the propagation gap from manifest to deliverable. This is a deliverable-completeness rule, not a re-judgment rule — the agent does not re-classify the WARN at deliverable time; the agent transcribes the audit-time classification (now Governor-disposed at independence 3 per §12.14 / startup Group 3B coverage-mode escalation rule).
